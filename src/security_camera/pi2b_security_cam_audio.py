...
# Version: v2 timing-fix; duplicates frames to keep recordings real-time on Pi 2 B
# =========================
# Designed for:
# - Raspberry Pi 2 B
...
active_recorder_next_write = 0.0
is_recording = False
talk_mode = False
...
def camera_loop():
    ...
    global latest_jpeg, latest_frame, camera_ok, camera_error, last_snapshot, active_recorder_next_write
    ...
    while True:
        ...
        try:
            ...
            while True:
                ...
                # Important: on a Pi 2 B the webcam may only deliver a few real frames/sec
                # while Flask is also streaming. If we write only one frame per camera read,
                # ffmpeg receives too few frames and the 30s clip becomes a ~5s fast-forward file.
                # Therefore we write duplicate frames according to wall-clock time so the MP4
                # timeline remains real-time even when capture FPS drops.
                with recorder_lock:
                    rec = active_recorder
                    next_write = active_recorder_next_write
                ...
                if rec is not None and rec.poll() is None:
                    try:
                        ...
                        writes = 0
                        max_catchup_writes = FRAMERATE * 2

                        if next_write <= 0:
                            next_write = now_mono

                        while now_mono >= next_write and writes < max_catchup_writes:
                        ...
                        with recorder_lock:
                            if active_recorder is rec:
                                active_recorder_next_write = next_write
...
def record_clip():
    global active_recorder, active_recorder_next_write, is_recording, last_motion
    ...
    with recording_lock:
        ...
        try:
            ...
            with recorder_lock:
                ...
                active_recorder_next_write = time.monotonic()
